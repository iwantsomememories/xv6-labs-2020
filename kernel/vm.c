...
// Copy user mappings to kernel page-table of process.
void
upt2kpt_copy(pagetable_t pt, pagetable_t kpt, uint64 oldsz, uint64 newsz)
{
  pte_t *pte_from, *pte_to;
  oldsz = PGROUNDUP(oldsz);
  for(uint64 i = oldsz; i < newsz; i += PGSIZE){
    if((pte_from = walk(pt, i, 0)) == 0)
      panic("upt2kpt_copy: pte_from should exist");
    if((pte_to = walk(kpt, i, 1)) == 0)
      panic("upt2kpt_copy: pte_to walk failed");
